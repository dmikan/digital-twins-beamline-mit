...
OBJECTIVE = "best-10% splat distance + transmission term (mm)"
DIRECTION = "minimize"
BAD_SCORE = 1.0e6  # returned when SIMION fails or records no ions
...
# either.
STUDY_NAME = "orchestrator_loop"
RESULTS_DB = HERE / "beamline_study_orchestrator.db"
...
def run_simion_candidate(voltages: np.ndarray) -> tuple:
    ...
    -------
    (value, hits, mean_all):
        value    -- the objective, lower is better: mean splat distance
                    of the closest SPLAT_TOP_FRACTION of ions, plus
                    HIT_TERM_SCALE_MM * (1 - hit fraction) so ranking
                    hands over to transmission once the leading edge
                    reaches the detector (see HIT_TERM_SCALE_MM).
                    BAD_SCORE on SIMION failure or if no ions recorded.
        hits     -- ion count inside DETECTOR_REGION (kept for the
                    NARROW_AFTER_HITS gate and reporting, no longer the
                    objective itself).
        mean_all -- all-ion mean splat distance, mm (stored in
                    user_attrs for reporting/diagnostics).
    """
    ...
    try:
        ...
        return BAD_SCORE, 0, BAD_SCORE

    positions = op.get_positions(out)
    if positions.shape[0] == 0:
        return BAD_SCORE, 0, BAD_SCORE
    hits = int(op.count_hits(positions))
    value = (mean_splat_distance(positions, SPLAT_TOP_FRACTION)
             + HIT_TERM_SCALE_MM * (1.0 - hits / positions.shape[0]))
    return value, hits, mean_splat_distance(positions)
...
def orchestrate(total_simion_budget, simion_per_iteration, target_score=None,
    ...
    while simion_spent < total_simion_budget:
        ...
        print(f"  Promoting top {len(top_k_idx)} (RK4 predicted objective "
              f"{min(promoted_scores):.1f} to {max(promoted_scores):.1f}mm) to real SIMION"
        ...
        for rank, idx in enumerate(top_k_idx, 1):
            ...
            t0 = time.time()
            real_value, simion_hits, mean_all = run_simion_candidate(voltages)
            simion_spent += 1
            elapsed = time.time() - t0
            user_attrs = {
                # RK4's stage-B prediction of the SIMION objective (mm,
                # lower better) -- directly comparable to the trial value.
            ...
            print(f"    [{rank}/{k}] rk4_pred={score_by_idx[idx]:.1f}mm -> SIMION="
                  f"{real_value:.1f}mm (all-ion {mean_all:.1f}mm), hits={simion_hits}"
        ...
        print(f"  Best real SIMION result so far: best-10% dist={best.value:.1f}mm, "
              f"hits={best.user_attrs.get('simion_hits', '?')} (trial {best.number})")

        # direction is minimize now: target_score is the mean splat distance
        # (mm) considered good enough to stop at.
    ...
    best = study.best_trial
    print(f"Best: {OBJECTIVE}={best.value:.1f}  (hits={best.user_attrs.get('simion_hits', '?')})")
