...
from physics import (
    IonSpecies, BatchBasisFieldMap, make_batch_beam, BatchTrajectory, BatchRK4Integrator, build_wall_index
)
from caracterizador import make_beam, BeamProgressScorer

DETECTOR_BBOX = op.DETECTOR_BBOX
WALL_EXCLUDE = op.WALL_EXCLUDE
SCORE_WEIGHTS = op.SCORE_WEIGHTS
SPLAT_TOP_FRACTION = op.SPLAT_TOP_FRACTION
HIT_TERM_SCALE_MM = op.HIT_TERM_SCALE_MM
N_ELECTRODES = 19
DT = op.DT
...
VARIANTS = [
    ("new m=0.5         ", False, 0.5, "new+wall100", False),
    ("new m=0.3         ", False, 0.3, "new+wall100", False),
    ("new m=0.2         ", False, 0.2, "new+wall100", False),
    ("new m=0.1         ", False, 0.1, "new+wall100", False),
    ("new m=0.0         ", False, 0.0, "new+wall100", False),
]
...
def main():
    ...
    fis = op.cargar_fisica(HERE, verbose=True)
