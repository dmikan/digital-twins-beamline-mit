...
from physics import BatchBasisFieldMap, make_batch_beam, BatchTrajectory, BatchRK4Integrator
from caracterizador import make_beam
from physics import build_wall_index
from optimizer import STUDY_NAME, RESULTS_DB
N_ELECTRODES = 19

optuna.logging.set_verbosity(optuna.logging.WARNING)

HERE = ROOT  # datos (CSVs, STLs) en la raiz
DETECTOR_BBOX = (69.0, 81.0, 68.5, 81.5, 403.0, 407.0)
WALL_EXCLUDE = (1, 2, 19)

# 1e-8, no el historico 5e-8: con la correccion de unidades el haz va a
# ~470 eV (ver orchestrator.py DT). 3000 pasos = ~2x el transito completo.
DT = 1e-8
NUM_STEPS = 3000  # full fidelity -- only 6 candidates, cheap enough
...
def main():
    ...
    out_path = ROOT / "outputs" / "cycle_best_comparison.png"
    plt.tight_layout()
