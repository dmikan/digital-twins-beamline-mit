...
class GemeloDigital:
    ...
    def fisica(self):
        if self._bfm is None:
            print("[gemelo] cargando campos base (19 CSVs)...")
            self._bfm = BatchBasisFieldMap.from_directory(HERE, n_electrodes=orc.N_ELECTRODES)
            print("[gemelo] construyendo indice de paredes...")
            self._wall = build_wall_index(HERE, exclude=orc.WALL_EXCLUDE, target_spacing=2.0)
    ...
    def mejor_global(self):
        """Mejor trial POR HITS a traves de TODOS los estudios archivados
        (studies/ y legacy/studies/) -- util cuando el estudio de esta
        instancia recien empieza pero ya existe conocimiento previo."""
        import glob
        import optuna
        optuna.logging.set_verbosity(optuna.logging.WARNING)
        mejor = None
        dbs = sorted(glob.glob(str(HERE / "studies" / "beamline_study_*.db"))) + \
              sorted(glob.glob(str(HERE / "legacy" / "studies" / "beamline_study_*.db")))
        for db in dbs:
            try:
...
if __name__ == "__main__":
    ...
    m = tw.mejor()
    if m is None:
        # PRIMERA VERSION DEL MODELO v2: si el estudio no existe, correr
        # gemelo.py lo entrena desde cero (como hacia fresh_run), con el
        # punto inicial fisico inyectado y el objetivo J_v2 normalizado.
