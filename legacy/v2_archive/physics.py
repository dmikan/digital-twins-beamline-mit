...
def _cargar_caja(root, nombre, archivos):
    ...
    print(f"[Campo1mm] cache construido: {cache.name} ({basis_V.nbytes / 1e6:.0f}MB)")
    return _MapaArrays(X, Y, Z, basis_V)

class Campo1mm(BatchBasisFieldMap):
    """
    Field map representation working in 1mm (unified resolution equivalent to SIMION).
    Route queries to fine boxes (quad, c1, c2) or coarse global map.
    """
    def __init__(self, basis_files, cajas_finas, root=ROOT):
        ...
        for nombre, archivos in cajas_finas:
            ...
            print(f"[Campo1mm] caja '{nombre}': {mapa.Nx}x{mapa.Ny}x{mapa.Nz} ({mapa.dx:.1f}mm) x[{lo[0]:g},{hi[0]:g}]")
...
def cargar_fisica(root=ROOT, n_electrodes=19, verbose=True):
    ...
    if tiene_dual:
        if verbose:
            print("[physics] unified 1mm resolution (Campo1mm + ParedesPA, margen 0.5)")
        bfm = Campo1mm.desde_proyecto(root, n_electrodes=n_electrodes)
        wall = ParedesPA.desde_proyecto(root, verbose=verbose)
        return Fisica(bfm, wall, MARGEN_1MM, CHUNK_1MM, True)
