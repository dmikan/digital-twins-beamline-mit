...
from caracterizador import (caracterizar, objetivo_v2, desde_simion_ultimo_vuelo,
                            cinematica_en_plano, puntaje_cinematico)
...
WALL_EXCLUDE = (1, 2, 19)
# Margen de contacto con pared, en mm. Lo fija cargar_fisica() al entrar
# a orchestrate(): 0.5 con la fisica dual del PA ("toco = casi entro al
# metal"), 1.5 con las paredes STL (el parche para su residuo de
# alineacion). Ver dual_grid.MARGEN_DUAL/MARGEN_STL.
WALL_HIT_MARGIN = 1.5
...
# 100 -> 25 (2026-07-06): existe un config confirmado MUY por encima del
# piso de ruido -- la cadena de colimacion V3=250/quad*0.70/V6=-750 dio
# 43 hits, con meseta estable 32-43 en V6=[-850,-750] (13 vuelos SIMION,
# playpen/afinar_foco_postquad.py). Con 25+ hits la senal es real y el
# bloque de explotacion fina alrededor del mejor trial debe encenderse.
NARROW_AFTER_HITS = 1
...
# 15 -> 8 (2026-07-05): corriendo desde gemelo.py (que ya carga su propio
# mapa de campos) el pico de la etapa B a chunk 15 agoto la RAM en
# _target_distance (MemoryError pidiendo 68MB = techo alcanzado). El pico
# escala lineal con el chunk; a 8 entra con margen. Mas chunks = mismo
# trabajo total, solo mas iteraciones del loop externo.
RESCREEN_CHUNK = 8
...
# Termino CINEMATICO del screening (2026-07-05): la trayectoria RK4 trae
# velocidad en cada paso, asi que el estado de fase se mide en planos a
# LO LARGO DE TODA LA TRAYECTORIA con caracterizador.cinematica_en_plano
# -- SOLO sobre particulas que cruzan cada plano ANTES de tocar pared,
# asi el sobre-marcado de paredes del RK4 no contamina la medicion (la
# leccion del intento J_v2-en-screening). puntaje_cinematico in [0,1]
# castiga divergencia/|alfa|/emitancia y premia haces colimados; el
# ranking usa el PROMEDIO sobre KIN_PLANES, asi que un candidato solo
# puntua bien si el haz va colimado de punta a punta, no solo al llegar.
# Entra acotado en mm-equivalentes (mismo orden que el termino de
# transmision, menor que el de pared). Validacion contra los vuelos
# SIMION archivados: playpen/comparar_ranking_momento.py (un solo plano
# z=390 ya subia los hitters reales en top-30 de 5/14 a 8/14).
#
# La linea tiene dos tramos: fuente->bender viajando en -x (x: 395->~75)
# y bender->detector viajando en +z (z: ~77->405). (eje, direccion,
# coordenada) por plano; ver cinematica_en_plano. El ranking usa el
# promedio sobre estos planos.
#
# POR QUE UN SOLO PLANO (calibrado 2026-07-05, playpen/calibrar_planos_
# kin.py, 150 sets archivados / 14 hitters): se probaron 5 planos
# cubriendo ambos tramos y CADA plano intermedio diluye o daña la senal
# -- x=330 no separa nada (kin hitters 0.404 = kin todos 0.404: cerca de
# la fuente todos los haces son el cono de 15 grados), z=200 y z=300
# solos rinden PEOR que sin momento (+0.281/+0.275 vs +0.303), y toda
# combinacion baja los hitters-en-top-30 de 8/14 (z=390 solo) a 5-7/14.
# La fisica: importa el estado que LLEGA; divergencia intermedia que la
# optica posterior corrige no es mala, y castigarla aleja al optimizador
# de soluciones validas. Confirmado con el barrido fino de 19 planos
# (playpen/buscar_planos_criticos.py): NINGUN plano intermedio mejora el
# combo con z=390 (todos <= 8/14). El punto intermedio donde los hitters
# MAS se distinguen es la entrada del cuadrupolo/bender (x=120,
# separacion +0.253) -- pero a fidelidad de screening (50 particulas)
# solo ~4 llegan ahi y la medicion se muere de hambre estadistica
# (agregarlo dio 6/14). Los planos intermedios quedan para DIAGNOSTICO
# (playpen/perfil_envolvente.py) y para recalibrar cuando haya mas
# hitters o mas particulas de screening.
KIN_PLANES = (
    (2, +1, 390.0),   # pre-detector: el unico plano que discrimina hitters
)
KIN_PENALTY_MM = 20.0
...
def rk4_score_chunk(bfm, wall_index, voltages_chunk, start_positions, start_velocities,
    ...
    + KIN_PENALTY_MM * puntaje_cinematico (si kinematica=True).
    ...
    kinematica=True agrega el termino de momento: estado de fase medido
    sobre los cruces limpios (antes de pared) en TODOS los planos de
    KIN_PLANES -- ambos tramos de la linea -- y promediado, asi el rank
    exige colimacion de punta a punta; ver KIN_PLANES/KIN_PENALTY_MM's
    comments. detalle=True devuelve la tripleta
    (total, base, puntaje_kin) en lugar del total solo -- es lo que usa
    playpen/comparar_ranking_momento.py para comparar ambos rankings con
    UN solo vuelo RK4 por chunk.
    """
    ...
    if kinematica:
        # (T, n_configs, n, 3): make_batch_beam agrupa particulas por
        # config (config-major), el reshape es directo. Las velocidades se
        # extraen POR CONFIG (float32, ~(T,n,3) transitorio de unos MB)
        # en vez de materializar el array completo: a fidelidad de
        # rescreen el array entero son ~100MB y esta funcion corre en el
        # pico de memoria del chunk (ver RESCREEN_CHUNK's comment).
        ...
        for c in range(n_configs):
            ...
            # colimacion de punta a punta: promedio sobre TODOS los planos
            # de KIN_PLANES (un plano no alcanzado puntua 1.0, asi que
            # morir a mitad de linea tambien cuesta aqui, no solo en el
            # termino de transmision)
            puntajes = [
...
def orchestrate(total_simion_budget, simion_per_iteration, target_score=None,
    ...
    # Fisica: dual 1mm (CampoDual + ParedesPA) si basis_quad/ existe,
    # clasica (bases 50^3 + STL) si no. El margen de contacto y el chunk
    # de screening van atados a la fisica cargada -- ver dual_grid.
    global WALL_HIT_MARGIN
    fis = cargar_fisica(HERE, n_electrodes=N_ELECTRODES)
