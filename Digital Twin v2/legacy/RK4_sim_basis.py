...
# beam_progress_score se importa solo dentro del demo de __main__ (abajo):
# la fisica no debe depender del calificador -- ver docs/GUIA_CODIGO.txt.
...
# Guarded under __main__ so importing this module (e.g. `from RK4_sim_basis import
# Beam, Trajectory, RK4Integrator, ...` from another script) doesn't trigger a full
# 500-particle/5000-step integration as a side effect of the import.
if __name__ == "__main__":
    import beam_progress_score as bps
    directory = './'
    ...
    Voltages_test = [500.0, 0.0, -895.921163747548, 0.0, 0.0, 689.815550200552, 0.0, 0.0 , -33.2956871479055, 778.366815914241, 331.740232885476, -352.050845144708, 0.0, 0.0, 376.220581070828, 0.0, 0.0, -123.329027787132, -2000]
    bfm.set_voltages(Voltages_test)  # Example voltages; replace with actual values
    species = IonSpecies(mass=28 * 1.66053906660e-27, charge=1.602176634e-19)  # Example: Nitrogen ion
    start_points = np.array([[395.0, 75.0, 77.0]])  # Starting position for the beam
    start_positions, start_velocities = bps.make_beam(N=500, species=species, start_point=start_points,
                                                      mean_energy_eV=15.0, std_energy_eV=0.42466, half_angle_deg=15, seed=42)
    beam_multi = Beam(
        species=species,
        position=start_positions,
        velocity=start_velocities,
    )
    BeamTrajectory = Trajectory(beam_multi)
    integrator = RK4Integrator(bfm)
    print("Starting integration...")
    integrator.integrate(BeamTrajectory, dt=5e-8, num_steps=5000)
    positions_multi = np.array([state.position for state in BeamTrajectory.states])
    #print(positions_multi.shape)  # Print the final positions of the first 5 particles

    #==============================================================================================
    #                            Now that we have the positions of 500 particles,
    #                       we can use the BeamProgressScorer to evaluate their progress.
    #==============================================================================================

    beamScr = bps.BeamProgressScorer(bfm=bfm, Trajectory=BeamTrajectory, dt=5e-8, num_steps=5000)
    score = beamScr.score(Voltages_test)
    print("Beam progress score:", score["mean_progress"], "survival_rate:", score["survival_rate"])
    beamScr2 = bps.BeamProgressScorer(bfm=bfm, Trajectory=BeamTrajectory, detector_bbox=(72.5, 79.5, 72.5, 79.5, 401.5, 408.5), dt=5e-8, num_steps=5000)
    score2 = beamScr2.score(Voltages_test)  # Initialize with zero voltages
    print("Beam progress score:", score2)
    #beamScorer.score()  # Initialize with zero voltages
