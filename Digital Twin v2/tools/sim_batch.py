...
HERE = ROOT  # datos (CSVs de campos, STLs) viven en la raiz del proyecto
N_ELECTRODES = 19
...
def sample_voltage_batch(m, seed=None):
    ...
    # DB propia en studies/ -- op.RESULTS_DB apuntaria a la raiz del
    # proyecto (optimizer.py no se toca), y esa DB vieja esta archivada.
    study = optuna.create_study(direction=op.DIRECTION,
                                 storage=f"sqlite:///{ROOT / 'studies' / 'beamline_study.db'}",
