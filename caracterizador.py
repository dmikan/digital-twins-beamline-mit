...
# ----------------------------------------------------------------------
# LAMBDA_HITS (objetivo v2.4): termino LINEAL de transmision perdida,
#   + LAMBDA_HITS * (1 - hits / n_total)
# anadido al J_v2. Ranking-equivalente a "J - lambda*hits/n" (solo
# desplazado en +lambda para conservar J >= 0 y haz perfecto = 0).
#
# POR QUE (diagnostico entre cuencas, 2026-07-10): el termino
# "transmision" saturante 1/(1+hits/H0) con H0=15 fue el fix correcto
# para el regimen 0-30 hits (ver playpen/validar_objetivo_v22.py), pero
# en el regimen actual (70-110 hits reales) un hit marginal vale
# ~0.0005 de J -- y NINGUN valor de H0 arregla eso (el credito maximo
# posible de +21 hits con esa forma es ~0.028, menor que la brecha
# cosmetica tipica entre cuencas, ~0.066 en offset+twiss+colim+halo).
# Resultado medido: la cuenca de Search v2.1/v2.2 (84 hits, J=0.170)
# le ganaba en J a la cuenca TES-GP-GP6D (105 hits, J=0.227). Este
# termino lineal NO satura: con lambda=2, +21 hits = 0.084 de J,
# decisivo entre cuencas, sin romper el refinamiento local (dentro de
# cuenca J y hits ya correlacionaban a -0.84). Verificado contra los
# trials reales: re-ordena correctamente 105 > 95 > 84 hits.
#
# El total se RE-NORMALIZA por (1+lambda), asi que J conserva la
# convencion [0,1] sin pared (~1.05 con pared). El rescale es uniforme:
# no altera ningun ordenamiento. Con lambda=2, +21 hits (de 500) vale
# ~0.032 de J normalizado vs ~0.022 de la brecha cosmetica tipica.
#
# lambda_hits=0 en objetivo_v2() reproduce el J_v2 anterior (v2.3) --
# OJO: valores J con lambda distinto NO son comparables entre si; usar
# estudios/DBs nuevos al cambiarlo.
# ----------------------------------------------------------------------
LAMBDA_HITS = 2.0
N_REF_HITS = 500.0   # fallback si features no trae n_total
...
# ----------------------------------------------------------------------
def objetivo_v2(features, con_pared=None, lambda_hits=None):
    """J_v2 (menor = mejor). lambda_hits=None usa LAMBDA_HITS (v2.4);
    lambda_hits=0 reproduce el J v2.3 anterior -- ver el bloque de
    comentario junto a LAMBDA_HITS para el diagnostico completo."""
    f = features
    hits = f.get("hits") or 0
    if lambda_hits is None:
        lambda_hits = LAMBDA_HITS
    ...
    if lambda_hits:
        # Normaliza por el haz COMPLETO (n_total), no por n_considerados:
        # con la mascara de limpias del RK4, hits/n_considerados puede dar
        # 1.0 con solo el 40% del haz en el detector.
        n_ref = f.get("n_total") or f.get("n_considerados") or N_REF_HITS
        frac = min(hits / max(1.0, float(n_ref)), 1.0)
        desglose["transmision_lineal"] = float(lambda_hits) * (1.0 - frac)
    ...
    if lambda_hits:
        # Re-normaliza TODO por (1+lambda) para conservar la convencion
        # J en [0,1] sin pared (con pared llega a ~1.05). Es un rescale
        # uniforme: NO cambia ningun ordenamiento -- lo que importa es la
        # RAZON entre el termino lineal de hits y los cosmeticos, y esa
        # se preserva. Con lambda=2 el peso efectivo queda: transmision
        # (lineal+saturante) ~0.83, cosmeticos ~0.09, cuerpo 0.033.
        norm = 1.0 + float(lambda_hits)
        desglose = {k: v / norm for k, v in desglose.items()}
