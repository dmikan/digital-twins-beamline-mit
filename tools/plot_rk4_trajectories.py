...
study_name = "gemelo_v2_bender6d"  # default study name
hitter_mode = False
for arg in sys.argv:
    if arg.startswith("--study="):
        study_name = arg.split("=")[1]
    if arg == "--hitter":
        hitter_mode = True

OUT = ROOT / "outputs" / "report_figures" / study_name
OUT.mkdir(parents=True, exist_ok=True)


def simulate_best_config():
    # 1. Load best voltages from database (supports --study=name)
    global study_name, hitter_mode
    ...
    try:
        ...
        if hitter_mode:
            ...
            for t in valid_trials:
                ...
                if hits is None:
                    hits = t.value if (t.value >= 0) else 0
                if hits is not None and hits > max_hits:
                    max_hits = hits
                    best_trial = t
            print(f"Mejores voltajes (Best Hitter) cargados de {study_name}: {best_trial.params}")
        else:
            best_trial = min(valid_trials, key=lambda t: t.value)
            print(f"Mejores voltajes (Best Value) cargados de {study_name}: {best_trial.params}")
        voltajes = best_trial.params
    except Exception as e:
        # Fallback default voltages if study load fails
        print(f"Aviso: No se pudo cargar el estudio {study_name} ({e}). Usando voltajes por defecto.")
...
def plot_3d_trajectories(pos_history, limpias, voltajes):
    ...
    fig.tight_layout()
    suffix = "_best_hitter" if hitter_mode else ""
    fig.savefig(OUT / f"fig_rk4_trayectorias_3d{suffix}.png", dpi=150)
    plt.close(fig)
    print(f"ok  fig_rk4_trayectorias_3d{suffix}.png")
...
def plot_phase_space_evolution(pos_history, vel_history, limpias, pasos):
    ...
    fig.tight_layout()
    suffix = "_best_hitter" if hitter_mode else ""
    fig.savefig(OUT / f"fig_rk4_espacio_fases{suffix}.png", dpi=150)
    plt.close(fig)
    print(f"ok  fig_rk4_espacio_fases{suffix}.png")


def plot_best_voltages(voltajes):
    # If 6D, expand to physical 8-electrode voltages for the bar plot
    if any(k in voltajes for k in ("A", "B", "C")):
        reduced = {}
        for k, v in voltajes.items():
            if k.startswith("V"):
                try:
                    reduced[int(k[1:])] = v
                except ValueError:
        ...
        voltajes_plot = voltajes

    labels = ["V3\n(Einzel 1)", "V6\n(Einzel 2)", "V9\n(Bender 1)", "V10\n(Bender 2)",
              "V11\n(Bender 3)", "V12\n(Bender 4)", "V15\n(Defl X)", "V18\n(Defl Y)"]
    keys = ["V3", "V6", "V9", "V10", "V11", "V12", "V15", "V18"]
    values = [voltajes_plot[k] for k in keys]
    ...
    fig.tight_layout()
    suffix = "_best_hitter" if hitter_mode else ""
    fig.savefig(OUT / f"fig_mejores_voltajes{suffix}.png", dpi=150)
    plt.close(fig)
    print(f"ok  fig_mejores_voltajes{suffix}.png")


if __name__ == "__main__":
    # Check if a specific study was requested via command line
    has_study_arg = any(arg.startswith("--study=") for arg in sys.argv)
    
    if has_study_arg:
        # Run only the requested study
        OUT = ROOT / "outputs" / "report_figures" / study_name
        OUT.mkdir(parents=True, exist_ok=True)
        ...
        # Loop through all three active studies by default
        for name in ["gemelo_db_v2", "gemelo_v2_bender6d", "gemelo_v2_bender7d"]:
            study_name = name
            OUT = ROOT / "outputs" / "report_figures" / study_name
            OUT.mkdir(parents=True, exist_ok=True)
            print(f"\n>>> Generando trayectorias RK4 para: {study_name}...")
            try:
                pos_hist, vel_hist, limpias, pasos, voltajes = simulate_best_config()
                plot_3d_trajectories(pos_hist, limpias, voltajes)
                plot_phase_space_evolution(pos_hist, vel_hist, limpias, pasos)
                plot_best_voltages(voltajes)
                print(f"ok  Gráficos de trayectorias guardados en: {OUT}")
            except Exception as e:
                print(f"[-] Error procesando {study_name}: {e}")
